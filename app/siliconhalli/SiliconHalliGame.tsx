...
import { Terminal, Users, Cpu, Coffee, Zap, DollarSign, TrendingUp, Building, Server, Globe, Info, RefreshCw, RotateCcw } from 'lucide-react';
...
const SAVE_KEY = 'siliconhalli_v1';
...
export default function SiliconHalli() {
  ...
  }, [balance, mrr, equity, valuation, inventory, clickCount, logs, isLoaded]);
  ...
  const handleReset = () => {
    if (confirm("Are you sure you want to reset your progress? This cannot be undone.")) {
      ...
      setInventory({});
      setClickCount(0);
      setLogs(["Game reset. Good luck!"]);
    }
  };

  if (!isLoaded) {
    return <div className="min-h-screen bg-slate-900 flex items-center justify-center text-green-400 font-mono">Loading SiliconHalli...</div>;
  }
      ...
      <div className="h-16 bg-slate-800 border-b-4 border-black flex items-center justify-between px-4 lg:px-8 shadow-lg z-10">
        ...
        <div className="text-center hidden md:flex flex-col items-center">
            <div className="flex items-center space-x-2">
                <div className="relative w-8 h-8">
                     <Image src="/siliconhalli-logo.png" alt="SiliconHalli Logo" fill className="object-contain" />
                </div>
        ...
        <div className="flex items-center space-x-6">
          ...
          <div className="flex space-x-2">
            <button
              onClick={handleReset}
              className="p-2 bg-slate-700 hover:bg-red-600 text-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center"
              title="Reset Progress"
            >
              <RotateCcw size={20} />
            </button>
          <Link href="/siliconhalli/about" className="p-2 bg-slate-700 hover:bg-slate-600 text-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center space-x-2" title="About the Game">
              <Info size={20} />
            <span className="hidden sm:inline font-bold text-xs">ABOUT</span>
            </Link>
          </div>
        </div>
      </div>
